...
document.addEventListener('DOMContentLoaded', function(){
  ...
  previewWrapper.setAttribute('class', 'img-container preview');
  // 表示する画像を生成
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  // input要素を取得。name情報は検証ツールから確認すること！！
  const fileField = document.querySelector('input[type="file"][name="customer[image]"]');
  // input要素で値の変化が起きた際に呼び出される関数
  fileField.addEventListener('change', function(e){
  // 古いプレビューが存在する場合は削除(最後に記述する)
  const alreadyPreview = document.querySelector('.preview');
  ...
  // 画像を表示するためのdiv要素を生成
  const previewWrapper = document.createElement('div');
  previewWrapper.setAttribute('class', 'img-container rounded-circle preview');
  // 表示する画像を生成
  const previewImage = document.createElement('img');
  previewImage.setAttribute('class', 'preview-image');
  // setAttributeメソッドを用いて生成したimg要素のsrc属性へ変数blobを設定する
